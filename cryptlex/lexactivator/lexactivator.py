import ctypes
import lexactivator_native as LexActivatorNative
from lexstatus_codes import LexStatusCodes
from lexactivator_exception import LexActivatorException
...
class LexActivator:
	...
	def GetProductMetadata(key):
		...
		status = LexActivatorNative.GetProductMetadata(key, buffer, buffer_size)
	...
	def GetLicenseMetadata(key):
		...
		buffer_size = 256
		buffer = LexActivatorNative.get_ctype_string_buffer(buffer_size)
		status = LexActivatorNative.GetLicenseMetadata(key, buffer, buffer_size)
		if status != LexStatusCodes.LA_OK:
			raise LexActivatorException(status)
		return buffer.value
	...
	def GetLicenseMeterAttribute(name):
		...
		status = LexActivatorNative.GetLicenseMeterAttribute(ctypes.byref(allowed_uses), ctypes.byref(total_uses))
		if status == LexStatusCodes.LA_OK:
	...
	def GetLicenseExpiryDate():
		...
		status = LexActivatorNative.GetLicenseExpiryDate(ctypes.byref(expiry_date))
		if status == LexStatusCodes.LA_OK:
	...
	def GetLicenseUserMetadata(key):
		...
		buffer_size = 256
		buffer = LexActivatorNative.get_ctype_string_buffer(buffer_size)
		status = LexActivatorNative.GetLicenseUserMetadata(key, buffer, buffer_size)
		if status != LexStatusCodes.LA_OK:
			raise LexActivatorException(status)
		return buffer.value
	...
	def GetActivationMetadata(key):
		...
		buffer_size = 256
		buffer = LexActivatorNative.get_ctype_string_buffer(buffer_size)
		status = LexActivatorNative.GetActivationMetadata(key, buffer, buffer_size)
		if status != LexStatusCodes.LA_OK:
			raise LexActivatorException(status)
		return buffer.value
	...
	def GetActivationMeterAttributeUses(name):
		...
		"""
		uses = ctypes.c_uint()
		status = LexActivatorNative.GetActivationMeterAttributeUses(name, ctypes.byref(uses))
		if status == LexStatusCodes.LA_OK:
			return uses.value
		else:
			raise LexActivatorException(status)
	...
	def GetServerSyncGracePeriodExpiryDate():
		...
		"""
		expiry_date = ctypes.c_uint()
		status = LexActivatorNative.GetServerSyncGracePeriodExpiryDate(ctypes.byref(expiry_date))
		if status == LexStatusCodes.LA_OK:
			return expiry_date.value
		elif status == LexStatusCodes.LA_FAIL:
			return 0
	...
	def GetTrialActivationMetadata(key):
		...
		buffer_size = 256
		buffer = LexActivatorNative.get_ctype_string_buffer(buffer_size)
		status = LexActivatorNative.GetTrialActivationMetadata(key, buffer, buffer_size)
		if status != LexStatusCodes.LA_OK:
			raise LexActivatorException(status)
		return buffer.value
	...
	def GetTrialExpiryDate():
		...
		"""
		expiry_date = ctypes.c_uint()
		status = LexActivatorNative.GetTrialExpiryDate(ctypes.byref(expiry_date))
		if status == LexStatusCodes.LA_OK:
			return expiry_date.value
		elif status == LexStatusCodes.LA_FAIL:
			return 0
	...
	def GetLocalTrialExpiryDate():
		...
		"""
		expiry_date = ctypes.c_uint()
		status = LexActivatorNative.GetLocalTrialExpiryDate(ctypes.byref(expiry_date))
		if status == LexStatusCodes.LA_OK:
			return expiry_date.value
		elif status == LexStatusCodes.LA_FAIL:
			return 0
	...
	def CheckForReleaseUpdate(platform, version, channel, release_callback):
		...
		status = LexActivatorNative.CheckForReleaseUpdate(platform, version, channel, release_callback_fn)
		if LexStatusCodes.LA_OK != status:
			raise LexActivatorException(status)
	...
	def GenerateOfflineDeactivationRequest(file_path):
		...
		Returns:
			int: LA_OK, LA_FAIL
		"""
		status = LexActivatorNative.GenerateOfflineDeactivationRequest(file_path)
		if LexStatusCodes.LA_OK == status:
			return LexStatusCodes.LA_OK
		elif LexStatusCodes.LA_FAIL == status:
			return LexStatusCodes.LA_FAIL
	...
	def GenerateOfflineTrialActivationRequest(file_path):
		...
		Raises:
			LexActivatorException
		"""
		status = LexActivatorNative.GenerateOfflineTrialActivationRequest(file_path)
		if LexStatusCodes.LA_OK != status:
			raise LexActivatorException(status)
